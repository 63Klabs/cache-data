...
- **Force Refresh Option** - Added `forceRefresh` option in `connection.options` to bypass cache expiration and always fetch from origin. Sends conditional headers (ETag, If-Modified-Since) for bandwidth efficiency, falls back to stale cache on origin failure, and sets status to `"original:cache-update-forced"` after successful forced update. [Spec: 1-3-14-force-refresh-option](.kiro/specs/1-3-14-force-refresh-option/) from [Feature Request: Force Refresh Option for CacheableDataAccess.getData() #227](https://github.com/63Klabs/cache-data/issues/227)
- **Shared Cache Identifier** - Added `sharedCacheId` parameter to `Cache.init()` that allows multiple Lambda functions to share cached data by overriding the default per-function salt used in cache key hash generation. Supports configuration via parameter or `CACHE_SHARED_ID` environment variable with priority chain resolution. [Spec: 1-3-14-shared-cache-identifier](.kiro/specs/1-3-14-shared-cache-identifier/) from [Feature Request: Shared Cache Identifier for Cross-Function Cache Sharing #226](https://github.com/63Klabs/cache-data/issues/226)
- **In-Memory Only Cache Mode** - Added `inMemoryOnly` option to `Cache.init()` that allows the entire cache system to operate without DynamoDB, S3, or encryption infrastructure. When enabled, the existing InMemoryCache (L0 layer) becomes the sole storage mechanism. Supports configuration via parameter or `CACHE_IN_MEMORY_ONLY` environment variable. Useful for heartbeat services, development environments, and lightweight applications that don't need persistent caching. [Spec: 1-3-14-in-memory-only-cache-mode](.kiro/specs/1-3-14-in-memory-only-cache-mode/) from [Feature Request: In-Memory Only Cache Mode #224](https://github.com/63Klabs/cache-data/issues/224)
