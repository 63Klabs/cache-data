			...
			fc.assert(
				fc.property(
					...
					(method, path) => {
						...
						expect(authsAfterModification).toEqual(originalStaticAuth);
					}
				),
				{ numRuns: 100 }
			);
			...
			fc.assert(
				fc.property(
					...
					(modifications) => {
						...
						expect(JSON.stringify(authsAfterModification)).toBe(JSON.stringify(originalAuths));
					}
				),
				{ numRuns: 100 }
			);
