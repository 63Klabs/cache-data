				...
				fc.property(
					...
					(lastModifiedValue, etagValue) => {
						...
						if (etag === null || etag === undefined) {
							expect(testConnection.headers['if-none-match']).toBeUndefined();
						} else {
							expect(testConnection.headers['if-none-match']).toBe(etag);
						}
						...
						if (lastModified === null || lastModified === undefined) {
							expect(testConnection.headers['if-modified-since']).toBeUndefined();
						} else {
							expect(testConnection.headers['if-modified-since']).toBe(lastModified);
						}
