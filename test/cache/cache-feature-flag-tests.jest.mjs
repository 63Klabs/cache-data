...
describe("Cache Feature Flag", () => {
	...
	describe("Test In-Memory Cache Feature Flag", () => {
		...
		it("Test feature flag in Cache.info()", async () => {
			// After Cache.init() is called (in cache-tests.jest.mjs), 
			// we can verify the feature flag is present in info
			const info = Cache.info();

			// The useInMemoryCache property should exist
			expect(info).toHaveProperty('useInMemoryCache');
			expect(typeof info.useInMemoryCache).toBe('boolean');
		});
		...
		it("Test Cache.info() includes inMemoryCache when enabled", async () => {
			...
			if (info.useInMemoryCache === true) {
				expect(info).toHaveProperty('inMemoryCache');
				expect(info.inMemoryCache).toHaveProperty('size');
				expect(info.inMemoryCache).toHaveProperty('maxEntries');
				expect(info.inMemoryCache).toHaveProperty('memoryMB');
			}
		});
	...
	describe("Test Feature Flag Initialization Scenarios", () => {
		...
		it("Should accept useInMemoryCache parameter", () => {
			...
			expect(testInit).toHaveProperty('useInMemoryCache');
			expect(typeof testInit.useInMemoryCache).toBe('boolean');
		});
		...
		it("Should accept CACHE_USE_IN_MEMORY environment variable", () => {
			...
			expect(process.env.CACHE_USE_IN_MEMORY).toBe('true');
