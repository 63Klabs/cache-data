...
describe('Shell Command Security', () => {
	...
	it('should handle file paths with spaces safely', async () => {
		...
		try {
			...
			expect(error.message).toContain(pathWithSpaces);
		}
	});
	...
	it('should handle file paths with quotes safely', async () => {
		...
		try {
			...
			const isExpectedError = error.code === 'ENOENT' || error.code === 1;
			expect(isExpectedError).toBe(true);
			expect(error.message).toContain(pathWithDoubleQuotes);
		}
		...
		try {
			...
			const isExpectedError = error.code === 'ENOENT' || error.code === 1;
			expect(isExpectedError).toBe(true);
			expect(error.message).toContain(pathWithSingleQuotes);
		}
	});
	...
	it('should prevent command injection via redirection', async () => {
		...
		try {
			...
			const isExpectedError = error.code === 'ENOENT' || error.code === 1;
			expect(isExpectedError).toBe(true);
			expect(error.message).toContain(maliciousPathOutput);
		}
		...
		try {
			...
			const isExpectedError = error.code === 'ENOENT' || error.code === 1;
			expect(isExpectedError).toBe(true);
			expect(error.message).toContain(maliciousPathInput);
		}
	});
});
