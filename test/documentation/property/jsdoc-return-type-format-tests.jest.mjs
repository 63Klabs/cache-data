			...
			fc.assert(
				fc.property(
					...
					(returnType) => {
						...
						expect(hasProperPromiseNotation(returnType)).toBe(true);
						expect(hasProperArrayNotation(returnType)).toBe(true);
						expect(hasProperObjectNotation(returnType)).toBe(true);
					}
				),
				{ numRuns: 100 }
			);
		...
		it("should reject improper return type formats using property-based testing", () => {
			...
			for (const invalidFormat of invalidFormats) {
				if (invalidFormat.includes('Promise') && invalidFormat !== 'Promise<void>') {
					expect(hasProperPromiseNotation(invalidFormat)).toBe(false);
				}
				if (invalidFormat === 'Array') {
					expect(hasProperArrayNotation(invalidFormat)).toBe(false);
				}
			}
		});
	});
